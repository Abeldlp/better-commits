...
async function main(config: z.infer<typeof Config>) {
  ...
  if (config.check_status) {
    ...
    const staged_files = git_status.staged.reduce(
      (acc, curr, i: number) =>
        color.green(acc + curr + addNewLine(git_status.staged, i)),
      ""
    );
    p.log.success("Changes to be committed:\n" + staged_files);
    if (missing_files.length) {
      const unstaged_files = missing_files.reduce(
        (acc, curr, i: number) =>
          color.red(acc + curr + addNewLine(missing_files, i)),
        ""
      );
      ...
      const selected_for_staging = (await p.multiselect({
        ...
        options: [
          { value: ".", label: "." },
          ...missing_files.map((v) => ({ value: v, label: v })),
        ],
        required: false,
      })) as string[];
  ...
  if (config.check_ticket.confirm_ticket) {
    const user_commit_ticket = (await p.text({
      message: commit_state.ticket
        ? `Ticket / issue infered from branch ${color.dim("(confirm / edit)")}`
  ...
  if (continue_commit)
    simpleGit().commit(build_commit_string(commit_state, config, false));
}
